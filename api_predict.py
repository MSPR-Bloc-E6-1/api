...
def predict():
    ...
    # vérifie que le fichier est bien un fichier image
    if not request.files['file'].filename.lower().endswith(('.png', '.jpg', '.jpeg')):
        return jsonify({'error': 'Le fichier doit être une image au format PNG, JPG ou JPEG.'}), 400
